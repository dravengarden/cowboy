...
Deno.test("diagnostic logs use bounded server-side filters and cursor pagination", () => {
  assertEquals(
    ...
    `/api/logs?limit=25&kind=cache_anomaly%2Cprovider_error&severity=warning%2Cerror&agent=claude&from_ms=${
      String(10 * 86_400_000)
    }&to_ms=${String(40 * 86_400_000)}&cursor=next-page`,
  );
});
...
Deno.test("diagnostic logs do not poll or render an empty state beside request errors", async () => {
  ...
  assertEquals(
    source.includes("!error && !loading && logs.length === 0"),
    true,
  );
});
...
Deno.test("critical and error use distinct severity accents across log dots and filters", async () => {
  ...
  assertEquals(
    /accent:\s*SEVERITY_OPTIONS\.find\(\(option\) => option\.value === value\)\s*\?\.accent/
      .test(source),
    true,
  );
  assertEquals(
    source.includes("bgcolor: SEVERITY_ACCENT[entry.severity]"),
    true,
  );
});
