...
export function createImeAutoInsertVim(): {
  ...
  const autoInsert = Prec.highest(EditorView.domEventHandlers({
    compositionstart: (_event, view): boolean => {
      ...
      const region = view.dom.closest<HTMLElement>("[data-desktop-region]");
      if (region && region.dataset.desktopFocused !== "true") return false;
  ...
  const commandSink = ViewPlugin.fromClass(class {
    ...
    private readonly onKeyDown = (event: KeyboardEvent): void => {
      ...
      const region = this.view.dom.closest<HTMLElement>("[data-desktop-region]");
      if (region && region.dataset.desktopFocused !== "true") return;
