...
export function mcpIdentity(toolName: string, rawInput?: unknown, title = ""): McpIdentity | null {
...
export function toolVariantLabel({ provider, toolName, kind, title, rawInput }: {
  provider: string;
  toolName: string;
  kind: string;
  title: string;
  ...
  return `${providerName(provider)} · ${toolTypeName(provider, toolName, kind)}`;
}
...
export function toolCopyText({ title, toolName = "", rawInput, content }: {
  ...
  const mcp = mcpIdentity(toolName, rawInput, title);
  const mcpPrimary = mcp && [mcp.arguments.function, mcp.arguments.query, mcp.arguments.url]
