...
//
//  1. Signed Provider `host.tool_presentations` select a closed bespoke
//     renderer for exact upstream tool names (e.g. a todo list).
...
export interface ToolCtx {
  ...
  provider: string;
  /** Exact immutable Provider package identity pinned by the session. */
...
type ToolRendererId =
  ProviderUiManifest["host"]["tool_presentations"][number]["renderer"];

const DECLARED_RENDERERS: Record<ToolRendererId, Renderer> = {
  todo_list_v1: todoTool,
};

function declaredToolRenderer(ctx: ToolCtx): Renderer | undefined {
  const declaration = currentProviderEntry(
    ctx.provider,
    ctx.providerVersion,
    ctx.providerDigest,
  )?.manifest.host.tool_presentations.find((candidate) =>
    candidate.tool_name === ctx.toolName
  );
  return declaration ? DECLARED_RENDERERS[declaration.renderer] : undefined;
}
...
export function ToolBody({ ctx }: { ctx: ToolCtx }): React.JSX.Element {
  const renderer = declaredToolRenderer(ctx) ??
