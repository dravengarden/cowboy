...
func mapResult(display shellDisplay, err error) map[string]any {
	...
	for index, frame := range display.Frames {
		frames[index] = map[string]any{"launcher": frame.Launcher, "text": frame.Text, "language": frame.Language, "label": frame.Label, "dialect": frame.Dialect, "depth": frame.Depth, "marker": frame.Marker, "color": frame.Color}
	}
