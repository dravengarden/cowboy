...
interface AuditRow {
  ...
  status: "current" | "upgrade_available" | "integrity_mismatch" | "manual_git_review";
  source: string;
}
...
const providerIds = requested === "all"
  ? [...Deno.readDirSync(providersRoot)]
...
for (const providerId of providerIds) {
  ...
  const source = JSON.parse(await Deno.readTextFile(path)) as ProviderSource;
  if (source.id !== providerId) throw new Error(`Provider directory identity mismatch: ${providerId}`);
  for (const dependency of source.runtime.dependencies) {
    ...
    if (
      ...
      !current.dist.tarball || !candidate.version || !candidate.dist?.integrity
    ) {
      throw new Error(`npm registry metadata is incomplete for ${packageName}`);
    }
    const integrityMatches = current.dist.integrity === dependency.integrity &&
...
console.log(JSON.stringify({ audited_at: new Date().toISOString(), providers: providerIds, dependencies: rows }, null, 2));
if (rows.some((row) => row.status === "integrity_mismatch")) Deno.exit(2);

function npmPackageName(source: string): string | null {
  const match = /^https:\/\/registry\.npmjs\.org\/(.+)\/-\/[^/]+\.tgz$/.exec(source);
  return match?.[1] ? decodeURIComponent(match[1]) : null;
}

async function npmMetadata(packageName: string, selector: string): Promise<NpmMetadata> {
  const encodedName = encodeURIComponent(packageName);
  const encodedSelector = encodeURIComponent(selector);
  const response = await fetch(`https://registry.npmjs.org/${encodedName}/${encodedSelector}`, {
