...
Release each Plugin as an independent artifact. Keep Agent Provider transports,
adapters, gateways, model clients, and other implementation dependencies private
to the payload; Cowboy users install only a named Plugin version on a named
Machine.
...
For real database ingestion/query acceptance, additionally run
`just telemetry-victoria-conformance <matrix.json> <databases.json> <new-receipt.json>`.
Require every role pair against explicit immutable VictoriaLogs/Metrics/Traces
executables, actual correlated log/trace identities and cumulative metric
values, same-store graceful database reopen and no replay after Cowboy restart.
This gate permits only disposable storage and isolated loopback; never add a
production URL, credential or existing data-directory input. Keep the 45-flow
protocol/fault gate separate. Bind database versions to the intended target
independently; a supplied triple, synthetic Operator and graceful reopen do not
accept actual host policy, production authentication, crash durability or P2
cutover. See `docs/telemetry-victoria-conformance.md`.
...
- Treat `(plugin_id, plugin_version, artifact_digest)` as the release unit and
  `(machine_id, plugin_id)` as the installation slot. Provider identity exists
  only inside an Agent Plugin payload.
...
- For Code Intelligence schema 2 (Plugin SDK 1.6+), require its complete exact
  ...
  twenty-one checks, including live authenticated Code installation into an empty
  ...
  and separately completed retirement. Require receipt schema v8; historical
  ...
  Session read-route changes additionally require v7 check 20: exact complete
  core file paging with a non-BMP page boundary, then original-cursor refusal
  after authenticated Machine reconnection and Controller restart, with no
  additional Machine dispatch and `410/no-store/no-ETag`. The older v6 chain
  does not accept this read-route extension or Machine-owned filesystem identity.
  Buffered read-authority changes additionally require v8 check 21: hold the
  real core file reply, revoke its disposable original login through the product
...
For every first-party Plugin, the minimum package gate is:

```bash
nix develop -c just plugin-check
nix develop -c just check
```

Agent Plugin changes additionally run `nix develop -c just provider-check` for
the capability payload and private runtime graph.

`just plugin-build <plugin-id>` is the independent data-only package build.
It writes `dist/plugins/<plugin-id>/<plugin-id>.cowboy-plugin` plus an
...
4. Publish the package, adjacent signed release envelope, trusted public key,
   ...
   command must independently verify the Plugin signature before writing Catalog
   bytes:

   ```bash
   nix develop -c just plugin-publish <plugin-id> <catalog-directory> <publisher-public-key>
   ```
...
```bash
nix develop -c just plugin-sign <plugin-id> <publisher-private-key>
nix develop -c just plugin-verify <plugin-id> <publisher-public-key>
```
...
The refresh must return success and the primary `/api/plugins` response must
advertise the exact Plugin ID, kind, version, component release, package digest,
composite artifact digest, contract fingerprint, `release_state=ready`, and
platform matrix before the release is reported as available. Its `providers`
field is only the Agent capability projection. An embedded
`release_state=unbound` entry is not a release receipt.
...
prerequisite. Do not simulate a Plugin release by editing the legacy
