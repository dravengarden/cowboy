// Cowboy-owned framework-neutral reactive store component.
//
// A minimal reactive store — the shared contract every reactive thing in the
// Cowboy state implements: a per-device pref (`persisted`) or a sync client
// (via its `toStore` adapter). One contract → one `useStore` hook
// renders any of them. Framework-agnostic; no React.
