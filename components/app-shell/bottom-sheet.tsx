...
const DISMISS_TAP_MOVE_SLOP_PX = 10;
const DISMISS_TAP_MAX_MS = 800;
const RETARGETED_CLICK_GUARD_MS = 500;
const MOBILE_SHEET_DISMISS_BUTTON_PX = 46;
// FloatingActionIsland contributes 4px inline padding and a 1px border on
// each side. Keep the shell large enough that its centred button never spills
// into the clipped rim.
const MOBILE_SHEET_DISMISS_ISLAND_PX = MOBILE_SHEET_DISMISS_BUTTON_PX + 2 * (4 + 1);

/**
 * After a touch dismiss unmounts the sheet, Mobile Safari can still deliver
 * the compatibility click to whatever now occupies that point — sheet body
 * underneath an overlay footer, or the page behind the sheet. Swallow those
 * leftover mouse/click events at the document for one Safari click window.
 */
function swallowRetargetedClicks(ms = RETARGETED_CLICK_GUARD_MS): void {
  ...
  globalThis.setTimeout(remove, ms);
}

/**
 * Commit a stationary touch on pointerup. Prevent the touch pointer sequence's
 * compatibility click before closing: this button unmounts synchronously, so
 * Mobile Safari can otherwise retarget the delayed click to newly revealed
 * content underneath the sheet. Mouse and keyboard activation continue to use
 * the native click path.
 */
function useReliableDismissTap(onClose: () => void): {
  ...
  const startRef = useRef<{ pointerId: number; x: number; y: number; at: number } | null>(null);
  ...
  const onPointerDown = useCallback((event: ReactPointerEvent<HTMLButtonElement>): void => {
    ...
    // Cancelling the primary touch pointer sequence prevents WebKit from
    // synthesizing a later mouse/click sequence after this button unmounts.
    event.preventDefault();
    startRef.current = {
  ...
  const onPointerMove = useCallback((event: ReactPointerEvent<HTMLButtonElement>): void => {
    ...
    event.preventDefault();
    if (
      Math.abs(event.clientX - start.x) > DISMISS_TAP_MOVE_SLOP_PX ||
      Math.abs(event.clientY - start.y) > DISMISS_TAP_MOVE_SLOP_PX
    ) {
      startRef.current = null;
    }
  }, []);

  const onPointerUp = useCallback((event: ReactPointerEvent<HTMLButtonElement>): void => {
    event.stopPropagation();
    const start = startRef.current;
    startRef.current = null;
    if (
      ...
      event.timeStamp - start.at > DISMISS_TAP_MAX_MS
    ) {
      return;
    }
    event.preventDefault();
    ...
    swallowRetargetedClicks();
    onClose();
  }, [clearSuppressionLater, onClose]);

  const onPointerCancel = useCallback((event: ReactPointerEvent<HTMLButtonElement>): void => {
    event.stopPropagation();
    startRef.current = null;
  }, []);

  const onClick = useCallback((event: ReactMouseEvent<HTMLButtonElement>): void => {
    event.stopPropagation();
              ...
              return (
                ...
                >
                  <ButtonBase
                    ...
                    onClick={(event) => {
                      // MobileSheetActionGroup is used as an inline overlay
                      // footer. Do not let an action also activate the page
                      // component that owns the sheet.
                      event.stopPropagation();
                      handlePress();
                    }}
                    onPointerDown={(event) => event.stopPropagation()}
  ...
  return (
    ...
    >
      <ButtonBase
        ...
        sx={{
          color: "text.primary",
          width: MOBILE_SHEET_DISMISS_ISLAND_PX,
          maxWidth: "100%",
          borderRadius: 999,
          display: "block",
      ...
      >
        <FloatingActionIsland maxWidth="100%" minHeight={MOBILE_SHEET_DISMISS_ISLAND_PX}>
          <Box
            aria-hidden
            sx={{
