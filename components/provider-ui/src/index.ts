...
export const AGENT_RUNTIME_BINDING_SCHEMA_VERSION = 2 as const;
...
export const PROVIDER_SDK_VERSION = "3.1.2" as const;
...
export interface ProviderContractInventory {
  ...
  min_runtime_binding_schema: number;
  max_runtime_binding_schema: number;
...
/** Generic Machine installation inventory. Agent Provider UI consumes a
 * filtered projection of this Plugin-owned lifecycle model. */
export interface MachinePluginInventory {
  ...
  active_session_leases: number;
  auth_generation?: number;
  replica_state: MachineProviderInventory["replica_state"];
  materialization_state: MachineProviderInventory["materialization_state"];
  detail?: string;
}
...
export function validateProviderCatalog(
  ...
  for (const raw of input.authentication_executors) {
    ...
    const identity =
      `${raw.provider_id}:${raw.provider_version}:${raw.generation_digest}`;
...
export function validateMachinePluginInventory(
  input: unknown,
): MachinePluginInventory[] {
  if (!Array.isArray(input)) {
    throw new Error("Invalid Machine Plugin inventory");
  }
  const identities = new Set<string>();
  for (const raw of input) {
    if (
      !isRecord(raw) || typeof raw.plugin_id !== "string" ||
      ...
      (raw.auth_generation !== undefined &&
        (typeof raw.auth_generation !== "number" ||
          ...
          raw.auth_generation < 1)) ||
      !["absent", "pending", "storing", "current", "failed", "revoking"]
        .includes(String(raw.replica_state)) ||
      !["not_installed", "applying", "current", "failed"].includes(
        String(raw.materialization_state),
      ) ||
      (raw.detail !== undefined &&
      ...
      throw new Error("Invalid Machine Plugin inventory entry");
    }
    if (identities.has(raw.plugin_id)) {
      throw new Error("Duplicate Machine Plugin inventory entry");
    }
    identities.add(raw.plugin_id);
  }
  return input as MachinePluginInventory[];
}

export function projectAgentPluginInventory(
  input: unknown,
): MachineProviderInventory[] {
  return validateMachinePluginInventory(input)
    .filter((plugin) => plugin.plugin_kind === "agent_provider")
    .map(({ plugin_id, plugin_version, plugin_kind: _kind, ...plugin }) => ({
      ...plugin,
...
export function validateProviderContractInventory(
  ...
  if (
    ...
    !hasOnlyKeys(input, [
      ...
      "min_package_schema",
      "max_package_schema",
      "min_runtime_binding_schema",
      "max_runtime_binding_schema",
      "min_ui_schema",
      "max_ui_schema",
      "min_host_schema",
      "max_host_schema",
...
export function providerCompatibilityProblem(
  ...
  if (
    AGENT_RUNTIME_BINDING_SCHEMA_VERSION <
      inventory.min_runtime_binding_schema ||
    AGENT_RUNTIME_BINDING_SCHEMA_VERSION > inventory.max_runtime_binding_schema
  ) {
    return {
      code: "release_schema_unsupported",
      detail: update(
        `Agent runtime binding schema ${AGENT_RUNTIME_BINDING_SCHEMA_VERSION}`,
      ),
    };
  }
