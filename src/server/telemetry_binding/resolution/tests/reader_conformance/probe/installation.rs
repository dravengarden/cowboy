...
fn snapshot(root: &Path) -> Result<serde_json::Value, Failure> {
    ...
    let mut statement = db.prepare("SELECT intent, intent_sha256, phase, problem, attention_from, created_at_ms, updated_at_ms FROM plugin_install_operations")
        .map_err(|_| Failure::EvidenceChanged)?;
...
async fn observe(address: std::net::SocketAddr, case: InstallCase) -> Result<(), Failure> {
    ...
    if report["schema"] != "dravengarden.cowboy.plugin-install-history/v1"
        || report["requires_reconciliation"] != fenced
        || report["execution_authorized"] != false
        || report["machine_receipt_available"] != false
    ...
    let problem = match phase {
        ...
        InstallPhase::Aborted => Some(InstallProblem::PreconditionsChanged),
