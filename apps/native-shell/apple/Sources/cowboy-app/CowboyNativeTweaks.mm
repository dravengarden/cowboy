...
//
// Owned source: tools/build-native-shell.sh stages this together with the current
// Passkey bridge into a fresh generated Xcode project for every build.
...
// Script handlers survive navigation. Treat a subframe or a foreign document
// as untrusted even if it directly calls a message handler without our shim.
static BOOL cowboyTrustedShellMessage(WKScriptMessage *message) {
...
- (void)userContentController:(WKUserContentController *)ucc
    ...
    if (!cowboyTrustedShellMessage(message)) return;
...
- (void)userContentController:(WKUserContentController *)ucc
    ...
    (void)ucc;
    if (!cowboyTrustedShellMessage(message)) {
        replyHandler(nil, @"Untrusted Cowboy native clipboard origin");
        return;
    }
