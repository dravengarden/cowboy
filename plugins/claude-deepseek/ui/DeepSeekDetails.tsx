...
import {
  ...
  percentLabel,
} from "../../../web/src/activityUsage";
import {
  type JsonRecord,
  num,
  type ProviderUsage,
  record,
} from "../../../web/src/usageLimits";
import {
...
import {
  ...
  TimeRangeButton,
} from "../../../web/src/ObservabilityFilters";
import {
  type ObservabilityTimeRange,
  timeRangeLabel,
  timeRangeQuery,
  validTimeRange,
} from "../../../web/src/observabilityTimeRange";
import { Sheet } from "../../../web/src/Sheet";
...
export function DeepSeekDetails(
  ...
  const [modelFilters, setModelFilters] = useState<string[]>(() =>
    storedDeepSeekMultiFilter("cowboy.deepseek.model", activityModelIds)
  );
  const [agentFilters, setAgentFilters] = useState<string[]>(() =>
    storedDeepSeekMultiFilter("cowboy.deepseek.agent", activityAgentIds)
  );
  ...
  useEffect(() => {
    ...
    void fetch(
      `/api/usage/${encodeURIComponent(usage.provider)}/activity?${query.toString()}`,
  ...
  }, [timeRange, modelFilters, agentFilters, usage.observed_at_ms, usage.provider]);
    ...
    <Stack spacing={1.15}>
      {balanceAccounts.map((account, index) => {
        ...
        const agents = Array.isArray(account.agents)
          ...
          : [];
        const lanes = agents.map((agent) => agentName(agent, usage.provider)).join(" + ");
        return (
          <Box
        ...
        <ActiveFilterChips
          items={[
            ...modelFilters.map((value) => ({
              key: `model:${value}`,
              label: modelOptions.find((option) => option.value === value)?.label ?? value,
              color: modelOptions.find((option) => option.value === value)?.color,
              onDelete: () => updateModels(modelFilters.filter((item) => item !== value)),
            })),
            ...agentFilters.map((value) => ({
              key: `agent:${value}`,
              label: agentOptions.find((option) => option.value === value)?.label ?? value,
              color: agentOptions.find((option) => option.value === value)?.color,
      ...
      >
        <Stack spacing={2} sx={{ pt: 0.5, pb: 1 }}>
          <MultiSelectChipGroup label="Model" options={modelOptions} value={draftModels} onChange={setDraftModels} />
          <MultiSelectChipGroup label="Runtime" options={agentOptions} value={draftAgents} onChange={setDraftAgents} />
            ...
            >
              <Stack spacing={0.55}>
                <Stack direction="row" justifyContent="space-between" alignItems="baseline" spacing={1}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    Auto · base {usageCacheIntervalLabel(usage.provider)} · verified ≥{usageCacheMinHitLabel(usage.provider)}
                  </Typography>
                </Stack>
                <Box
                  sx={{
                ...
                {cacheKeepaliveRequests > 0
                  ...
                  : (
                    <Typography variant="caption" color="text.secondary">
                      No keepalive attempts in this window. Eligible snapshots start after a verified ≥90% cache hit with at least {usageCacheMinHitLabel(usage.provider)} hit tokens. Protection spend is separate from agent spend.
                    </Typography>
                  )}
              </Stack>
            </Box>
                  ...
                  if (!totals || !cache || (num(totals.requests) ?? 0) === 0) {
                    return (
                      <Box key={agent} sx={{ borderRadius: 1.5, bgcolor: "action.hover", px: 1.1, py: 0.9 }}>
                        <Stack spacing={0.35}>
                          <Typography variant="body2" fontWeight={700}>{agentName(agent, usage.provider)}</Typography>
                          <Typography variant="caption" color="text.secondary">
              ...
              <AccordionDetails sx={{ px: 0, pt: 0 }}>
                <Stack spacing={0.75}>
                  ...
                  {byAgent && Object.entries(byAgent).map(([agent, value]) => {
                    ...
                    const cost = activityCostStats(record(costByAgent?.[agent]));
                    const durationObservations = num(totals?.durationObservations) ?? 0;
                            ...
                            <>
                              ...
                              <InfoRow
                                ...
                                v={cost.modelFamilies.length > 0
                                  ? cost.modelFamilies.map((family) => usageActivityModelLabel(family, usage.provider)).join(" + ")
                                  : "Unknown"}
                              />
                            </>
                          )}
                  ...
                  {lowHitByCause && Object.keys(lowHitByCause).length > 0 && (
                    <Stack spacing={0.35} sx={{ pt: 0.25 }}>
                      ...
                      {Object.entries(lowHitByCause).map(([cause, value]) => {
                        const totals = record(value);
                        const cost = activityCostStats(record(lowHitCostByCause?.[cause]));
                        return (
                          <InfoRow
                            key={cause}
