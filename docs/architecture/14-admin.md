...
The product `Outbound::Settings` snapshot is an empty compatibility tombstone.
`cowboy.admin.identities`, `cowboy.permissions`, `cowboy.session_limits`, and
the registration invite table never ride the product WebSocket. The retired
`Inbound::SetSetting` command is accepted and ignored for stale installed PWAs;
admin HTTP stays on `/api/admin/*`.
