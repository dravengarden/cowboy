...
组件闭包按 3.2.0 独立换代，Plugin 不变；不将本地 cleanup 视为跨端补偿。
第十六批 [IDB 连接 ownership](owned-idb-connections.md) 将全局连接缓存迁为显式数据库 owner 与事务租约，
...
| [Provider UI](../components/provider-ui/src/index.ts) / [authoring 与 owner](provider-ui-ownership.md) | 纯数据 authoring 已关联字段/消息/赋值/slot；核心 UI owner 同步准入与隔离迟到结果；[登录/确认流程](provider-management-ownership.md) 已接入有界观察与显式操作；通用 DAG DSL、其他领域投影与持久恢复仍待推进 |
