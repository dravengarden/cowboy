...
are explicitly opened; Desktop retains the visible diagnostic workbench.
Journal absence is not presented as evidence that remote export has stopped.
...
- Web release:
  `/nix/store/4pz8pjdzad7lqd3hhmflmv1nymwrn393-cowboy-web-release`.
...
Web tests, browser-runner format/type gates and clean Nix Web build passed.
Lint still reports three pre-existing non-blocking spread warnings in
`otel.ts` and `otelTransport.ts`.
...
StrictMode and MUI. At 360px width with 44px button targets the expanded recovery
was 443px high. It checks safe single-flight downloads, unchanged originals,
per-record failure, unavailable/empty inventory and late unmount/logout results.
