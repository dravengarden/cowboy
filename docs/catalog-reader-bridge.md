...
## Full successor ancestry and publication — 2026-09-07

The full Plugin successor now records `c0911dd012bad4191f2936491c3879346cffbd25`
as an ancestor for the component activator. Its three branch-only commits
contain only the legacy reader backport and this documentation. The ancestry
merge preserves the full successor's schema-2 reader and exact Host policy;
it does not reinstate schema-1 exclusions or the bridge's host-startup ban.

The user authorized publication without historical Provider coexistence as a
prerequisite. All six Agent Plugins 3.1.14 and Zed 1.2.2 are now published, with
new-version platform/runtime acceptance, independent signatures and complete
Catalog/URL verification. Historical Catalog bytes and failure receipts remain
intact. This does not claim native-history resume or old/new coexistence passed.

The live Controller is still the bridge. The full successor's read-only check
against actual Hawk configuration rejects the missing exact WebAuthn storage
Host selection. First Host activation needs its separate NixOS policy and
storage recovery transition; the existing component-only rollback cannot undo
that transition. See [the publication record](releases/plugin-publication-2026-09-07.md)
for exact released identities, evidence and remaining activation boundaries.
